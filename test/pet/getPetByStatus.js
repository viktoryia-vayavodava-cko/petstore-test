// get existing pet by existing status
...
feature.only('Verify the we can search by using staus', function () {
  ...
  scenario("Verify that correct pet is returned by staus provided", function () {
   ...
   // let context;
    ...
    Then(" this pet is returned", function () {
      ...
      let item = context.body.findIndex(i => i.id === parseInt(pet1.id));

      var valueAtIndex = context.body[item];

      expect(parseInt(pet1.id)).to.deep.equal(parseInt(valueAtIndex.id));
  ...
  scenario(" Getting existing pets by several statuses", function () {
   ...
   // let context;
    ...
    Then(" pets with that status are returned", function () {
      context.body.should.include.something.that.has.property("status", "available");
 //     context.body.should.include.something.that.has.property("status", "sold");
